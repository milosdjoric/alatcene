-- search_grouped: "ušteda" više nije raspon cena između prodavnica (max_cena - min_cena),
-- nego najveći akcijski POPUST na pojedinačnoj ponudi u grupi (redovna_cena - cena).
-- Realna ušteda je popust na jednom proizvodu, ne razlika cena između radnji.
--
-- Promene u odnosu na 20260423203438_search_grouped_ignore_sumnjiva:
--   1. _groups dobija kolonu max_popust = MAX(redovna_cena - cena) po grupi
--      (samo ponude na akciji koje nisu sumnjive) — ušteda u dinarima.
--   2. _groups dobija kolonu max_popust_procenat = MAX(popust_procenat) po grupi
--      — popust u procentima.
--   3. sort_by = 'usteda_desc' sada rangira po max_popust DESC (ušteda u RSD).
--   4. sort_by = 'popust_desc' ranije je (pogrešno) sortirao po min_cena ASC;
--      sada rangira po max_popust_procenat DESC (najveći % popusta).
--   5. JSON izlaz grupe sadrži 'max_popust' i 'max_popust_procenat'.
...
BEGIN
  ...
  SELECT
    ...
    -- Najveći akcijski popust u grupi (redovna_cena - cena) na pojedinačnoj ponudi.
    -- Samo ponude na akciji (redovna > cena) koje nisu sumnjive. 0 ako nema akcije.
    COALESCE(
    ...
    -- Najveći popust u procentima u grupi (za sort 'popust_desc').
    COALESCE(
